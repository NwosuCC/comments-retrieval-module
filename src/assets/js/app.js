...
function toJSON(result) {
    try{
        ...
        alert('JSON Parse Error: ' + JSON.stringify(e.message));
        return;
    }
    return result;
}
...
function submitForm(){
    ...
    // form.url = 'https://www.amazon.com/eero-Home-WiFi-System-Pack/dp/B00XEW3YD6/ref=sr_1_1?s=pc&ie=UTF8&qid=1489250467&sr=1-1&keywords=eero&th=1#customerReviews';
    // form.url = ' https://www.youtube.com/watch?v=BjZbAzUM9Ao';
...
function pollServer(){
    ...
    $.get(handler, params).done(function(result){
        ...
        // alert('done: ' + done + ' | status: ' + status + ' | poll: ' + poll);
        if(!done){
...
var CheckboxGroup = {
    ...
    create: function() {
        // var 'fields' will differ according to category
        // So, how to know the category? => Ajax on <textarea> blur()
        var fields = [
